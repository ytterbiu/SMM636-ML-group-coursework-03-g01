...
for (i in seq_along(rmd_lines)) {
  if (grepl("^```\\{", rmd_lines[i])) { in_chunk <- TRUE; keep[i] <- FALSE; next }
  if (in_chunk && grepl("^```\\s*$", rmd_lines[i])) { in_chunk <- FALSE; keep[i] <- FALSE; next }
